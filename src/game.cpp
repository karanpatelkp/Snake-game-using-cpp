...
Game::Game(const Game& other)
    : snake(std::make_unique<Snake>(*other.snake)),
      food(std::make_unique<SDL_Point>(*other.food)),
      engine(dev()), // Reinitialize engine with current object's dev
...
Game& Game::operator=(const Game& other) {
    ...
    snake = std::make_unique<Snake>(*other.snake);
    food = std::make_unique<SDL_Point>(*other.food);
    engine = std::mt19937(dev()); // Reinitialize engine with current object's dev
    random_w = other.random_w;
    random_h = other.random_h;
    score = other.score;
    return *this;
